filename=exp_ModelNet_run_two_phase_np
...
### Shuffle seed ###
seed=2026  # Random seed for data shuffle

### PPM sampling seed ###
ppm_seed=42

### Name of the directory to save ###
savedirname="result/${dataset}_UW_rips${rips}_tr${toporep}_dtm${dtm}_PPM_repeat${nb_repeat}_dim${dim}_bs${bs}_pointnet${pointnet}_deepsets${deepsets}_pointmlp${pointmlp}_np${num_points}_seed${seed}_ppmseed${ppm_seed}"

mkdir -p $savedirname
python3 $filename.py $savedirname $dataset $rips $toporep $dtm $nb_repeat $dim $bs $pointnet $deepsets $pointmlp $num_points $seed $ppm_seed 2>&1 | tee $savedirname/train.log
