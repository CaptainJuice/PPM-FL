...
class PersLay(nn.Module):
    def __init__(self, num_labels, **kwargs):
        ...
        self.theta = nn.Parameter(torch.rand(self.num_obsrv, 2) * 4).to('cuda:0')
        # device=kwargs.get("device", "cpu")
        # print('theta device ',device)
        # self.fc = nn.Linear(self.num_obsrv, num_labels)
        self.fc = nn.Sequential(nn.Linear(self.num_obsrv, num_labels), nn.ReLU())
        self.fc = self.fc.to('cuda:0')
    ...
    def forward(self, points_list):
        ...
        for L in points_list:
            if len(L) > 0:
                ...
                out_list.append(torch.zeros(self.num_obsrv).to('cuda:0'))
        # for x in out_list:
        #     print(x.device)
        out = torch.stack(out_list, dim=0)
        # print('aaaaaa',out.device)
        out = self.fc(out)   
        return out