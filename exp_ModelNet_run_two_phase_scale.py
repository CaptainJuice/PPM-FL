...
if __name__ == "__main__":
    ...
    all_X = pointcloud_normalize(all_X[:, :64, :])
    # print(all_X.shape)
    ...
    for CV_idx in range(CV_num):
        ...
        for epoch in range(epoch_num):
            ...
            epoch_loss_list = []
            idx_list = random.sample(range(train_num), train_num)
            for idx in tqdm(range(train_num//batch_size)):
                opt.zero_grad()
                batch_idx_list = idx_list[batch_size * idx: batch_size * (idx+1)]
    ...
    print(np.mean(time_lst),'±',np.std(time_lst))
