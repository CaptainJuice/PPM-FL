...
if __name__ == "__main__":
    ...
    if outlier_perct>0:
        ...
        all_X = add_uniform_noise_to_point_clouds(all_X,outlier_perct)
    # print(all_X.shape)
    ...
    for CV_idx in range(CV_num):
        ...
        dnn_pretrained_model_path = 'result/ModelNetNoisy01_C=10,N=100,T=1,K=2000_rips0_tr1_dtm0_PPM_repeat200_dim-1_bs40_pointnet0_deepsets1_pointmlp0'
        ...
        if toporep:     
            toporep_net = ExpTopoRep(num_points, feature_dim, 
                                  ...
                                  input_format="dist",device=device)
        ...
        toporep_net.load_state_dict(torch.load(f"{dnn_pretrained_model_path}/toporep_CVidx={CV_idx}.pth"))
        for param in toporep_net.parameters(): param.requires_grad = False

        feature_net.load_state_dict(torch.load(f"{dnn_pretrained_model_path}/feature_net_CVidx={CV_idx}.pth"))
        for param in feature_net.parameters(): param.requires_grad = False

        task_solver.load_state_dict(torch.load(f"{dnn_pretrained_model_path}/task_solver_CVidx={CV_idx}.pth"))
        for param in task_solver.parameters(): param.requires_grad = False
    ...
    print(np.mean(test_acc_lst),'±',np.std(test_acc_lst))
            #     print(f"Initial valid_loss = {valid_loss}, Initial valid_acc = {valid_acc}")
            # else:
